...
\begin{align}
  \fdv{L}{(\dot{\phi}(\xb))}=\lim_{a\to 0}\pdv{L}{\dot{\phi}_{\nb}}
\end{align}
となるものです。
...
これを踏まえてPoisson括弧を考えてみましょう。$A,B$を$\pi(\xb),\phi(\xb)$の汎関数をします。上の処方をPoisson括弧でやると
\begin{align}
  \{A,B\}=\int d^3\xb\qty(
...
\begin{align}
  \ddot{\phih}(x)-\bnabla^2\phih(x)+\frac12 m^2\phih(x)=0.
\end{align}
...
となります。ですから
\begin{align}
  \ah(\kb):=\sqrt{\frac{2\omega(\kb)}{(2\pi)^3}}
\end{align}
とすると
\begin{align}
